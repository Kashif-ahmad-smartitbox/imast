    ...
    <div className="relative rounded-3xl overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-500 ease-out">
      ...
      <div className="relative h-96 lg:h-[480px] overflow-hidden">
        {post.image ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-primary-500/10 to-primary-600/20 flex items-center justify-center">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-black/20 pointer-events-none z-[1]" />

        {/* Featured Badge */}
        {post.featured && (
          <div className="absolute top-6 right-6 z-20">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-lg backdrop-blur-sm">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-white text-xs sm:text-sm">
              {post.date && (
                <span className="flex items-center gap-1.5 sm:gap-2 bg-white/25 backdrop-blur-md px-2.5 sm:px-3 py-1.5 rounded-full border border-white/20">
                  <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 text-white" />
                  <span className="text-white">{formatDate(post.date)}</span>
                </span>
              )}
              {post.readTime && (
                <span className="flex items-center gap-1.5 sm:gap-2 bg-white/25 backdrop-blur-md px-2.5 sm:px-3 py-1.5 rounded-full border border-white/20">
                  <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 text-white" />
...
const PostCard = ({ post }: { post: Post }) => (
  <article className="bg-white rounded-2xl overflow-hidden transition-all duration-300 group border border-gray-100 hover:border-primary-200 hover:shadow-xl cursor-pointer h-full flex flex-col">
    ...
    <div className="p-5 sm:p-6 flex flex-col flex-grow">
      ...
      {/* Excerpt */}
      <p className="text-sm text-gray-600 mb-4 sm:mb-6 leading-relaxed line-clamp-3 group-hover:text-gray-700 transition-colors duration-300 flex-grow">
        {post.excerpt}
      </p>
      ...
      <div className="flex items-center justify-between pt-4 sm:pt-5 border-t border-gray-100 mt-auto gap-2">
        ...
        <a
          href={post.slug}
          className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-primary-600 font-semibold text-sm bg-primary-50 hover:bg-primary-100 hover:gap-3 transition-all duration-300 group/btn flex-shrink-0"
